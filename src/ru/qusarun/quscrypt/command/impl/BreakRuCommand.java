...
@CommandInfo(usage = "breakru <message>", description = "automatically decrypts Russian messages")
public class BreakRuCommand extends Command {
    ...
    public String decrypt(String message) {
        if (StringUtil.isValidRussian(message))
                ...
                if (cipher.getKeyType() == KeyType.NONE) {
                    try {
                        ...
                        if (result.equalsIgnoreCase(message))
                            continue;
                        if ((cipher.producesNonASCII() && StringUtil.isValidRussianString(result)) || StringUtil.isValidRussian(result)) {

...
import ru.qusarun.quscrypt.cipher.impl.Vigenere;
...
@CommandInfo(usage = "break <message>", description = "automatically decrypts messages")
public class BreakCommand extends Command {
    ...
    public String decrypt(String message) {
        if (StringUtil.isValidEnglish(message))
                ...
                if (cipher.getKeyType() == KeyType.NONE) {
                    try {
                        ...
                        if (result.equalsIgnoreCase(message))
                            continue;
                        if ((cipher.producesNonASCII() && StringUtil.isValidString(result)) || StringUtil.isValidEnglish(result)) {
        ...
        int i = 0;
        final Vigenere vigenere = (Vigenere) QusCrypt.INSTANCE.getCipherManager().getCipherByName("Vigenere");
        for (final String word: StringUtil.EN_WORDS) {
            vigenere.setKey(word);
            final String result = vigenere.decrypt(message);
            if (result.equalsIgnoreCase(message))
                continue;
            if (++i % 2500 == 0)
                System.out.println("Tried " + i + " words for vigenere");
            if (StringUtil.isValidEnglish(result)) {

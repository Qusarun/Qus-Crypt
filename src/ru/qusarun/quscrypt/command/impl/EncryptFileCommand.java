...
@CommandInfo(usage = "encryptfile <file>", description = "encrypts a file with the selected cipher")
public class EncryptFileCommand extends Command { @Override public boolean execute(final String[] args) { return args.length < 1? invalidUsage() : QusCrypt.INSTANCE.getCipherManager().getCipher() == null? Logger.log("Select a cipher first") : Logger.log(QusCrypt.INSTANCE.getCipherManager().getCipher()._encryptFile(StringUtil.join(args))); } }

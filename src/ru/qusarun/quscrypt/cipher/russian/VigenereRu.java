...
public class VigenereRu extends Cipher {
    public VigenereRu() { super(); }
    ...
    public String transform(final String message, final boolean inv) {
        ...
        for (final char c: message.toLowerCase().toCharArray()) {
            if (CharUtil.RU_ALPHABET.contains(Character.toString(c)))
    ...
    public char map(final int i) { return CharUtil.RU_ALPHABET.charAt(i >= 0 ? i % CharUtil.RU_ALPHABET.length() : (26 + i % CharUtil.RU_ALPHABET.length()) % CharUtil.RU_ALPHABET.length()); }

    public String keyToLen(final String msg) { return strKey.repeat(msg.length() / strKey.length() + 1); }

    public int getAlphabetPos(final char c) { return CharUtil.RU_ALPHABET.indexOf(Character.toLowerCase(c)); }
}
